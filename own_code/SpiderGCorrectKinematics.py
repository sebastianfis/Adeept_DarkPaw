...
def position_ctrl(change_input):
    ...
    if change_input == 'Tforward':
        ...
        if global_position <= 1:
            global_position = 2
        elif 1 < global_position < 5:
            global_position = 5
        elif 4 < global_position < 8:
            global_position = 8
...
def status_GenOut(height_input, pitch_input, roll_input): # TODO: Adapt in RobotModel!!!!
    ...
    def leg_FL_status():
        ...
        goal_dict['FLM'] = ctrl_range(int(FLM_init_pwm + (height_input + FL_input) * FLM_direction), max_dict['FLM'],
                                      min_dict['FLM'])
        goal_dict['FLE'] = FLE_init_pwm

    def leg_FR_status():
        goal_dict['FRB'] = FRB_init_pwm
        goal_dict['FRM'] = ctrl_range(int(FRM_init_pwm + (height_input + FR_input) * FRM_direction), max_dict['FRM'],
                                      min_dict['FRM'])
        goal_dict['FRE'] = FRE_init_pwm

    def leg_HL_status():
        goal_dict['HLB'] = HLB_init_pwm
        goal_dict['HLM'] = ctrl_range(int(HLM_init_pwm + (height_input + HL_input) * HLM_direction), max_dict['FRM'],
                                      min_dict['FRM'])
        goal_dict['HLE'] = HLE_init_pwm

    def leg_HR_status():
        goal_dict['HRB'] = HRB_init_pwm
        goal_dict['HRM'] = ctrl_range(int(HRM_init_pwm + (height_input + HR_input) * HRM_direction), max_dict['FRM'],
                                      min_dict['FRM'])
